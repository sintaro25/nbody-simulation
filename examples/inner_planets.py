...
sim_rk = NBodySimulator(bodies[0:5], rk4_step, dt)
E_rk = sim_rk.run(1000)

sim_lf = NBodySimulator(bodies[0:5], leapfrog_step, dt)
E_lf = sim_lf.run(1000)

plt.plot(E_rk, label='RK4')
plt.plot(E_lf, label='Leapfrog')
plt.legend()
plt.title('Energy comparison — Inner planets')
out = os.path.join(repo_root, 'results', 'example_inner_planets.png')
plt.savefig(out)
print('Saved', out)